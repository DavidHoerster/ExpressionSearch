...
namespace Web.Common
{
    ...
    {
        public static Expression<Func<Repository.BatterDetail, bool>> 
            CreateSearchPredicate(GridOptions options)
        {
            ...
            var predicate = isGroupOperationAND ? 
                    PredicateBuilder.True<Repository.BatterDetail>() : 
                    PredicateBuilder.False<Repository.BatterDetail>();

            foreach (var rule in options.Filters.FilterRules)
            {
                var operationAttribute = ExtractOperationComparisonType(rule.Operation);

                PropertyInfo pi = typeof(BatterDetail).GetProperty(rule.Field);
                ParameterExpression lhsParam = Expression.Parameter(typeof(BatterDetail));
                ...
                var theLambda = Expression.Lambda<Func<BatterDetail, bool>>(theOperation, lhsParam);
